...
public class MotionTrackingActivity extends Activity implements View.OnClickListener {
    ...
    private double[] prevTrans = {0, 0, 0};
    private double xNorm, yNorm, x, y, magc;
    private Tango mTango;
    private TangoConfig mConfig;
    private TextView currT;
    private TextView prevT;
    private TextView prevC;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prevT = (TextView) findViewById(R.id.prevTextView);
        currT = (TextView) findViewById(R.id.curTextView);
        prevC = (TextView) findViewById(R.id.prevCor);
                ...
                while (true) {
                    try {
                        ...
                        if (startCounter == 1) {
                            Thread.sleep(2000);
                            for (int i = 0; i < 3; i++) {
                                prevTrans[i] = mPose.translation[i];
                            }
                            ...
                            if (counter == 0) {
                                ...
                                x = mPose.translation[0] - prevTrans[0];
                                y = mPose.translation[1] - prevTrans[1];

                                magc = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2));
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        prevT.setText("FORWARD");
                                        currT.setText("LEFT");
                                    }
                                });

                                if (Math.abs(xNorm) > 0.5) {
                                    runOnUiThread(new Runnable() {

                                        @Override
                                        public void run() {
                                            prevC.setText("   Incorrect!");
                                            prevC.setTextColor(Color.RED);
                                        }
                                    });
                                }
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        @Override
                                        public void run() {
                                            prevC.setText("   Correct!");
                                            prevC.setTextColor(Color.GREEN);
                                        }
                                    });
                                }

                                // gets the previous translation for computation
                                for (int i = 0; i < 3; i++) {
                                    prevTrans[i] = mPose.translation[i];
                                }
                            }
                            ...
                            // -x direction
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        prevT.setText("LEFT");
                                        currT.setText("BACKWARD");
                                    }
                                });

                                if (Math.abs(yNorm) > 0.5) {
                                    runOnUiThread(new Runnable() {
